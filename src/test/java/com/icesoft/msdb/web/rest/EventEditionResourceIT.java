...
/**
 * Integration tests for the {@link EventEditionResource} REST controller.
 */
@SpringBootTest(classes = MotorsportsDatabaseApp.class)
public class EventEditionResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static EventEdition createUpdatedEntity(EntityManager em) {
        EventEdition eventEdition = new EventEdition()
    ...
    public void getEventEdition() throws Exception {
        ...
        restEventEditionMockMvc.perform(get("/api/event-editions/{id}", eventEdition.getId()))
            ...
            .andExpect(jsonPath("$.shortEventName").value(DEFAULT_SHORT_EVENT_NAME))
            .andExpect(jsonPath("$.longEventName").value(DEFAULT_LONG_EVENT_NAME))
