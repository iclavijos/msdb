...
public class EventEntryResultResourceIntTest {
    ...
    public void setup() {
        MockitoAnnotations.initMocks(this);
            EventEntryResultResource eventEntryResultResource = new EventEntryResultResource(eventEntryResultRepository, eventEntryResultSearchRepository);
    ...
    public void createEventEntryResultWithExistingId() throws Exception {
        ...
        EventEntryResult existingEventEntryResult = new EventEntryResult();
        existingEventEntryResult.setId(1L);

        // An entity with an existing ID cannot be created, so this API call must fail
        restEventEntryResultMockMvc.perform(post("/api/event-entry-results")
            .contentType(TestUtil.APPLICATION_JSON_UTF8)
            .content(TestUtil.convertObjectToJsonBytes(existingEventEntryResult)))
