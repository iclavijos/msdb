...
export class RaceDataComponent implements OnInit { // , AfterViewInit {
  ...
  getAccumulatedRaceTime(lapNumber): number {
    const tmp = this.lapByLapDataSource.filteredData[0];
    return tmp[lapNumber].raceTime as number;
  }
  ...
  ngOnInit() {
          ...
          label(tooltipItem, data) {
            ...
            return
              `${data.datasets[tooltipItem.datasetIndex].label as string}
                ${this.timeMask.transform(data.datasets[tooltipItem.datasetIndex].data[tooltipItem.index] * 1000, true, false) as string}`;
          }
        }
      },
      scales: {
        ...
        yAxes: [
          {
            ...
            ticks: {
              ...
              callback(value, index, values) {
                ...
                return this.timeMask.transform(value * 1000, false, false) as string;
              }
            }
          }
        ]
    ...
    this.eventSessionService.findMaxLaps(this.session.id).subscribe(res => {
      ...
      newOptionsSlider.translate = function(value: number) {
        if (value === 0) {
          return this.translateService.instant('motorsportsDatabaseApp.lapbylap.grid') as string;
        }
        if (value === this.ceil) {
          return this.translateService.instant('motorsportsDatabaseApp.lapbylap.finish') as string;
        }
        return `${this.translateService.instant('motorsportsDatabaseApp.lapbylap.lap') as string} ${value as string}`;
      };
    ...
    this.eventSessionService.findRaceChartData(this.session.id).subscribe(res => {
      ...
      for (let racePosition = 1; racePosition <= raceParticipants; racePosition++) {
        this.lapByLapColumns.push({
          columnDef: `pos ${racePosition as string}`,
          header: `Pos ${racePosition as string}`,
          cell: (element: unknown) => element
        });
      }

      this.lapByLapDisplayedColumns = ['lap'];
      this.lapByLapColumns.map(c => c.columnDef as string).forEach(col => this.lapByLapDisplayedColumns.push(col));
      ...
      for (let i = 0; i < laps; i++) {
        ...
        this.raceChart[i].racePositions.forEach(rp => {
          driverPositionLap[`pos ${rp.position as string}`] = rp;
          driverPositionLap[`pos ${rp.position as string}`].driverName = this.drivers
      ...
      const graphicLabels = this.raceChart.map(lapRaceChart =>
        lapRaceChart.lapNumber === 0
          ? this.translateService.instant('motorsportsDatabaseApp.lapbylap.grid') as string
          : `${this.translateService.instant('motorsportsDatabaseApp.lapbylap.lap') as string} ${lapRaceChart.lapNumber as string}`
      );
      ...
      const labels = this.raceChart[0].racePositions.map(pos => `${pos.raceNumber as string} ${pos.driverName as string}`);
      this.optionsRaceChart = {
        tooltips: {
          callbacks: {
            label(tooltipItem, dataset) {
              const item = dataset.datasets[tooltipItem.datasetIndex];
              return `#${item.label as string} - Pos ${tooltipItem.value as string}`;
            }
          }
        },
        scales: {
        ...
        tooltips: {
          callbacks: {
            label(tooltipItem, dataset) {
              ...
              if (timeGap > 0) {
                return `${dataset.datasets[tooltipItem.datasetIndex].label as string} +${this.timeMask.transform(timeGap, true, false) as string}`;
              } else {
                return dataset.datasets[tooltipItem.datasetIndex].label as string;
              }
            }
          }
        },
          ...
          yAxes: [
            {
              ...
              ticks: {
                ...
                callback(value, index, values) {
                  ...
                  return this.timeMask.transform(value, false, false) as string;
                }
              }
            }
          ]
      ...
      labels.forEach(label => {
        ...
        const dataset = {
          label,
          data: this.raceChart
            .map(lapRaceChart => lapRaceChart.racePositions.filter(rp => `${rp.raceNumber as string} ${rp.driverName as string}` === label))
            .map(l => {
              if (l[0] !== undefined) {
                return l[0].position;
              }
        ...
        const gapsDataset = {
          ...
          data: this.raceChart.map(lapRaceChart => {
            const racePosition = lapRaceChart.racePositions.filter(rp => `${rp.raceNumber as string} ${rp.driverName as string}` === label)[0];
...
//   }
  ...
  private convertDriversNames(): DriversNames[] {
    const result: DriversNames[] = [];
  ...
  private convertDriverAverages(json: any): DriverAverages[] {
    const result: DriverAverages[] = [];
  ...
  private convertRaceChartData(json: any): LapPositions[] {
    const result: LapPositions[] = [];
    for (let i = 0; i < json.length; i++) {
      result.push(Object.assign(new LapPositions(), json[i]));
    }
    return result;
  }
}
