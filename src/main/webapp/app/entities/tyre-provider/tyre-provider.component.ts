...
export class TyreProviderComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.tyreProviderService
        ...
        .subscribe((res: HttpResponse<ITyreProvider[]>) => this.paginateTyreProviders(res.body, res.headers));
      return;
    }
    this.tyreProviderService
      .query({
  ...
  registerChangeInTyreProviders() {
    this.eventSubscriber = this.eventManager.subscribe('tyreProviderListModification', () => this.loadAll());
  }
  ...
  protected paginateTyreProviders(data: ITyreProvider[], headers: HttpHeaders) {
