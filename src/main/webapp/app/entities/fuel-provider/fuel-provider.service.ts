...
export class FuelProviderService {
  ...
  public typeaheadSearchUrl = SERVER_API_URL + 'api/_typeahead/fuel';
  ...
  typeahead(req?: string): Observable<ResponseWrapper> {
    return this.http.get(`${this.typeaheadSearchUrl}?query=${req}`).map((res: Response) => this.convertResponse(res));
  }
}
