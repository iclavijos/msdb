    ...
    this.activatedRoute.data.subscribe(({ eventEdition }) => {
      ...
      if (eventEdition.seriesEditions) {
        this.eventEditionService.findPrevNextInSeries(eventEdition.id)
          .subscribe(res => (this.navigationIds = res as number[]));
      }
      this.eventEditionService.hasLapsData(eventEdition.id).subscribe(res => (this.hasLapsData = res as boolean));
      this.eventService.findEventEditionIds(eventEdition.event.id).subscribe(res => {
        ...
        const currentEdPos = this.editions.map(e => e.id as number).indexOf(eventEdition.id);
