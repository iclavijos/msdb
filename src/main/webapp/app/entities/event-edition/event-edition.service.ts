...
export class EventEditionService {
  ...
  protected eventResourceUrl = 'api/events';
  ...
  findCalendarEvents(startDate: Date, endDate: Date): Observable<any[]> {
    ...
    const fromDate = this.datePipe.transform(startDate, dateFormat) as string;
    const toDate = this.datePipe.transform(endDate, dateFormat) as string;
