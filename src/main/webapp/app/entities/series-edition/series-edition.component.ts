...
export class SeriesEditionComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.seriesEditionService
        ...
        .subscribe((res: HttpResponse<ISeriesEdition[]>) => this.paginateSeriesEditions(res.body, res.headers));
      return;
    }
    this.seriesEditionService
      .query({
  ...
  registerChangeInSeriesEditions() {
    this.eventSubscriber = this.eventManager.subscribe('seriesEditionListModification', () => this.loadAll());
  }
  ...
  protected paginateSeriesEditions(data: ISeriesEdition[], headers: HttpHeaders) {
