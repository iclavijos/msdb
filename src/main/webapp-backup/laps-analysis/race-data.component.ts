...
export class RaceDataComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.eventSessionService
      .findSessionDriverNames(this.sessionId)
      .subscribe((res: Response) => (this.drivers = this.convertDriversNames(res.json)));
    this.eventSessionService
      .findSessionAverages(this.sessionId)
      .subscribe((res: Response) => (this.averages = this.convertDriverAverages(res.json)));
    this.eventSessionService.findFastestTime(this.sessionId).subscribe((res: Response) => {
      this.fastestTime = Number(res.json);
      this.options.scales.yAxes[0].ticks.min = Math.floor((this.fastestTime - 10000) / 10000);
      this.refreshGraphic();
    });
    this.eventSessionService.findMaxLaps(this.sessionId).subscribe((res: Response) => {
      this.maxLaps = Number(res.json);
      this.lapsRangeTo = this.maxLaps;
    });
    this.eventSessionService.findRaceChartData(this.sessionId).subscribe((res: Response) => {
  ...
  refreshLapTimesTable(raceNumber: string) {
    ...
    if (pos != -1) {
      //Driver selected
      this.eventEditionService.loadLapTimes(this.sessionId, raceNumber).subscribe((res: Response) => {
