...
export class DriverPointsSeriesDialogComponent implements OnInit {
  ...
  private subscribeToSaveResponse(result: Observable<any>) {
    result.subscribe((res: any) => this.onSaveSuccess(res), (res: Response) => this.onSaveError(res));
  }
...
export class DriverPointsSeriesPopupComponent implements OnInit, OnDestroy {
  ...
  constructor(private route: ActivatedRoute, private driverPointsDetailsPopupService: DriverPointsDetailsPopupService) {}

  ngOnInit() {
    this.routeSub = this.route.params.subscribe(params => {
      if (params['id']) {
        this.driverPointsDetailsPopupService.open(DriverPointsSeriesDialogComponent as Component, null, null, params['id']);
      } else {
        this.driverPointsDetailsPopupService.open(
          DriverPointsSeriesDialogComponent as Component,
