...
export class TyreProviderResolve implements Resolve<ITyreProvider> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<ITyreProvider> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        filter((response: HttpResponse<TyreProvider>) => response.ok),
        map((tyreProvider: HttpResponse<TyreProvider>) => tyreProvider.body)
      );
    }
